...
public class GuiPersonalDetails extends GuiGetServer{
    ...
    public void initGui(){
        ...
        while (matcher.find()) {
            ...
            {
                if (component == null)
                    component = new ChatComponentText(part);
                else
                    component.appendText(part);
            }

            lastEnd = end;
            IChatComponent link = new ChatComponentText(matcher.group(1));
            ChatStyle style = link.getChatStyle();
            style.setChatClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, matcher.group(2)));
            style.setColor(EnumChatFormatting.BLUE);
            style.setUnderlined(true);
            style.setChatHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ChatComponentText(Util.localize("order.url"))));
    ...
    private IChatComponent getComponent(int mouseX, int mouseY)
    {
        int stringWidth = fontRendererObj.getStringWidth(info2.getFormattedText());
        int begin = (width / 2) - (stringWidth / 2);

        if (info2 instanceof ChatComponentStyle)
        {
            ChatComponentStyle comp = (ChatComponentStyle) info2;

            int prevWidth = begin;

            for (Object innerNoCast : comp)
            {
                IChatComponent inner = (IChatComponent) innerNoCast;
                StringBuilder stringbuilder = new StringBuilder();
                String s = inner.getUnformattedTextForChat();

                if (!s.isEmpty())
                {
                    stringbuilder.append(inner.getChatStyle().getFormattingCode());
