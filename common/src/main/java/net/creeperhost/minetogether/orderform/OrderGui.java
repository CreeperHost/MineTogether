...
public class OrderGui implements GuiProvider {
    ...
    public void buildGui(ModularGui gui) {
        ...
        gui.setGuiTitle(new TranslatableComponent("minetogether:gui.order.title"));
    ...
    private void setupSummaryPanel(GuiElement<?> background) {
        ...
        GuiElement<?> lastElement = new GuiText(scrollPane, new TranslatableComponent("minetogether:gui.order.summary").withStyle(ChatFormatting.UNDERLINE, ChatFormatting.GOLD))
                .constrain(TOP, relative(scrollPane.get(TOP), 4))
                .constrain(LEFT, left)
                .constrain(RIGHT, right)
                .constrain(HEIGHT, literal(8));

        new GuiText(scrollPane, () -> new TextComponent(summary.summaryError).withStyle(ChatFormatting.YELLOW))
        ...
        //Server Details
        lastElement = new GuiTextList(scrollPane, () -> summary.serverFeatures.stream().map(TextComponent::new).toList())
                .setScroll(false)
                .setLineSpacing(1)
        ...
        //Location
        Component locText = new TranslatableComponent("minetogether:gui.order.summary.location");
        ...
        new GuiText(lastElement, new TranslatableComponent("I").withStyle(UNDERLINE, GREEN))
                .setEnabled(() -> summary.summaryError.isEmpty())
                .setTooltipSingle(this::fallbackToolTip)
                .setTooltipDelay(0)
                .setScroll(false)
        ...
        lastElement = new GuiText(scrollPane, () -> new TextComponent(summary.serverHostName))
                .setEnabled(() -> summary.summaryError.isEmpty())
                .setTextColour(GREEN.getColor())
                .constrain(TOP, relative(lastElement.get(BOTTOM), 2))
                .constrain(LEFT, left)
        ...
        //Includes
        lastElement = new GuiText(scrollPane, new TranslatableComponent("minetogether:gui.order.summary.features"))
                .setEnabled(() -> summary.summaryError.isEmpty())
                .constrain(TOP, relative(lastElement.get(BOTTOM), 4))
                .constrain(LEFT, left)
                .constrain(RIGHT, right)
        ...
        for (int i = 1; i < 6; i++) {
            lastElement = new GuiText(scrollPane, new TranslatableComponent("minetogether:gui.order.summary.feature" + i))
                    .setEnabled(() -> summary.summaryError.isEmpty())
                    .setTextColour(GREEN.getColor())
        ...
        //Paying for
        lastElement = new GuiText(scrollPane, new TranslatableComponent("minetogether:gui.order.summary.paying_for"))
                .setEnabled(() -> summary.summaryError.isEmpty())
                .setWrap(true)
                .constrain(TOP, relative(lastElement.get(BOTTOM), 4))
                .constrain(LEFT, left)
        ...
        lastElement = new GuiText(scrollPane, new TranslatableComponent("minetogether:gui.order.summary.paying_for_details1"))
                .setEnabled(() -> summary.summaryError.isEmpty())
                .setTextColour(GREEN.getColor())
                .setWrap(true)
                .constrain(TOP, relative(lastElement.get(BOTTOM), 2))
        ...
        lastElement = new GuiText(scrollPane, new TranslatableComponent("minetogether:gui.order.summary.paying_for_details2"))
                .setEnabled(() -> summary.summaryError.isEmpty())
                .setTextColour(GREEN.getColor())
                .setWrap(true)
        ...
        //Cancel any time
        lastElement = new GuiText(scrollPane, new TranslatableComponent("minetogether:gui.order.summary.cancel_any_time"))
                .setEnabled(() -> summary.summaryError.isEmpty())
                .setWrap(true)
                .constrain(TOP, relative(lastElement.get(BOTTOM), 4))
                .constrain(LEFT, left)
    ...
    private Component fallbackToolTip() {
        return new TranslatableComponent("minetogether:gui.order.summary.location_fallback", new TextComponent(getDCName(order.serverLocation)).withStyle(GOLD), new TextComponent(getDCName(computeFallbackLocation())).withStyle(GOLD));
    }
    ...
    private void setupPricePanel(GuiElement<?> background) {
        ...
        GuiText total = new GuiText(background, new TranslatableComponent("minetogether:gui.order.summary.total"))
        ...
        GuiText totalValue = new GuiText(background, () -> new TextComponent(summary.prefix + String.format("%.2f", summary.total) + " " + summary.suffix))
                .setAlignment(Align.RIGHT);
        Constraints.bind(totalValue, total);

        GuiText tax = new GuiText(background, new TranslatableComponent("minetogether:gui.order.summary.tax"))
        ...
        GuiText taxValue = new GuiText(background, () -> new TextComponent(summary.prefix + String.format("%.2f", summary.tax) + " " + summary.suffix))
                .setEnabled(() -> summary.tax != 0)
                .setAlignment(Align.RIGHT);
        Constraints.bind(taxValue, tax);

        GuiText discount = new GuiText(background, new TranslatableComponent("minetogether:gui.order.summary.discount"))
        ...
        GuiText discountValue = new GuiText(background, () -> new TextComponent(summary.prefix + String.format("%.2f", summary.discount) + " " + summary.suffix))
                .setEnabled(() -> summary.discount != 0)
                .setAlignment(Align.RIGHT);
        Constraints.bind(discountValue, discount);

        GuiText subTotal = new GuiText(background, new TranslatableComponent("minetogether:gui.order.summary.sub_total"))
                .setAlignment(Align.LEFT)
                .constrain(LEFT, relative(background.get(LEFT), 2))
                .constrain(RIGHT, relative(background.get(RIGHT), -2))
    ...
    private void buildProcessingScreen(GuiElement<?> root) {
        ...
        processingButton = MTStyle.Flat.button(background, TextComponent.EMPTY)
        ...
        MTStyle.Flat.button(background, new TranslatableComponent("minetogether:gui.button.close"))
    ...
    public void emailDirty() {
        ...
        if (!EMAIL_PATTERN.matcher(order.emailAddress.toLowerCase()).matches()) {
            emailMessage = new TranslatableComponent("minetogether:gui.order.email_invalid");
            return;
        }
        emailCheckTimer = 60;
    ...
    private void tick(ModularGui gui) {
        if (!nameValid && --nameCheckTimer == 0) {
            if (order.name.isEmpty()) {
                nameMessage = new TranslatableComponent("minetogether:gui.order.blank.name");
            } else {
        ...
        if (!emailValid && --emailCheckTimer == 0) {
            if (order.emailAddress.isEmpty()) {
                emailMessage = new TranslatableComponent("minetogether:gui.order.blank.email");
            } else {
    ...
    private Component getOrderButtonText() {
        if (orderTask != null) {
            return new TranslatableComponent("minetogether:gui.order.order_in_progress");
        } else if (summaryUpdating || summaryUpdateRequired) {
            return new TranslatableComponent("minetogether:gui.order.summary.updating");
        } else if (inputsValid) {
            return new TranslatableComponent("minetogether:gui.order.place_order");
        } else {
            return invalidMessage == null ? TextComponent.EMPTY : invalidMessage.copy().withStyle(ChatFormatting.RED);
        }
    }
