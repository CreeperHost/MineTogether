...
public class MineTogetherConnect {
    ...
    private static void onScreenOpen(Screen screen, ScreenAccess screenAccess) {
        ...
        if (!Config.instance().moveButtonsOnPauseMenu || feedBack == null || options == null) {
            ...
            Button guiButton = new Button(screen.width - 105, 25, 100, 20, new TranslatableComponent("minetogether.connect.open"), (button) -> Minecraft.getInstance().setScreen(new GuiShareToFriends(screen)));
            ScreenHooks.addRenderableWidget(screen, guiButton);
            return;
        }

        // Open To Friends button goes where the options button was.
        Button guiButton = new Button(options.x, options.y, 98, 20, new TranslatableComponent("minetogether.connect.open"), (button) -> Minecraft.getInstance().setScreen(new GuiShareToFriends(screen)));
