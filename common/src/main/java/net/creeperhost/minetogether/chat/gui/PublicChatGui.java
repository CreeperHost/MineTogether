...
public class PublicChatGui implements GuiProvider {
    ...
    public void buildGui(ModularGui gui) {
        ...
        gui.setGuiTitle(new TranslatableComponent("minetogether:gui.chat.title"));
        ...
        GuiText connectionStatus = new GuiText(root)
                .setTooltipSingle(() -> {
                    ...
                    return new TextComponent("(" + ChatConstants.STATE_DESC_LOOKUP.get(state) + ")").withStyle(ChatConstants.STATE_FORMAT_LOOKUP.get(state));
                })
                .setTooltipDelay(0)
                ...
                .setTextSupplier(() -> {
                    IrcState state = MineTogetherChat.CHAT_STATE.ircClient.getState();
                    return new TextComponent(connectIndicator(state)).withStyle(ChatConstants.STATE_FORMAT_LOOKUP.get(state));
                });
    ...
    private void tick() {
        ...
        if (state != IrcState.CONNECTED) {
            ...
            textField.setValue("");
            textField.setSuggestion(new TranslatableComponent(ChatConstants.STATE_SUGGESTION_LOOKUP.get(state)));
            return;
        }
