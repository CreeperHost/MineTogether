        ...
        addRenderableWidget(menuDropdownButton = new DropdownButton<>(-1000, -1000, 100, 20, new TranslatableComponent("Menu"), new Menu(strings), false, p ->
        {
            ...
            if (menuDropdownButton.getSelected().option.equalsIgnoreCase(I18n.get("minetogether.chat.button.mute")))
            {
                ...
                if (profile != null)
                {
                    ...
                    ChatHandler.addStatusMessage("Locally blocked " + activeDropdown);
                }
            }
            else if (menuDropdownButton.getSelected().option.equalsIgnoreCase(I18n.get("minetogether.chat.button.addfriend")))
            {
                try
                {
                    minecraft.setScreen(new FriendRequestScreen(new ChatScreen(parent), Minecraft.getInstance().getUser().getName(), KnownUsers.findByDisplay(activeDropdown), ChatCallbacks.getFriendCode(MineTogetherClient.getPlayerHash()), "", false, false));
                } catch (IOException e)
                {
    ...
    public boolean handleComponentClicked(@Nullable Style style, double mouseX, double mouseY)
    {
        ...
        if (event.getAction() == ClickEvent.Action.SUGGEST_COMMAND)
        {
            ...
            if (eventValue.contains(":"))
            {
                ...
                Minecraft.getInstance().setScreen(new FriendRequestScreen(this, Minecraft.getInstance().getUser().getName(), targetProfile, friendCode, friendName, true, false));
                return true;
            }
