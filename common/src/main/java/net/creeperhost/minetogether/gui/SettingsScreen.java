...
public class SettingsScreen extends Screen {
    ...
    protected void init() {
        ...
        addRenderableWidget(new Button(width / 2 - 123, 40, 120, 20, Component.translatable("minetogether:screen.settings.button.chat").append(state(config.chatEnabled)), e -> {
        ...
        addRenderableWidget(new Button(width / 2 + 3, 40, 120, 20, Component.translatable("minetogether:screen.settings.button.friend_toasts").append(state(config.friendNotifications)), e -> {
            config.friendNotifications = !config.friendNotifications;
            saveConfig();
        }));
        addRenderableWidget(new Button(width / 2 - 123, 60, 120, 20, Component.translatable("minetogether:screen.settings.button.menu_buttons").append(state(config.mainMenuButtons)), e -> {
            config.mainMenuButtons = !config.mainMenuButtons;
            saveConfig();
        }));

        linkButton = addRenderableWidget(new Button(width / 2 - 100, height - 47, 200, 20, Component.translatable("minetogether:screen.settings.button.link"), e -> {
        ...
        addRenderableWidget(new Button(width / 2 - 100, height - 27, 200, 20, Component.translatable("gui.done"), e -> {
