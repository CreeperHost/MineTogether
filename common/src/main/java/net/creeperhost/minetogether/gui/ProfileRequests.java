...
public class ProfileRequests {
    ...
    public static MutableComponent getBanId() {
        ...
        return new TextComponent(ban == null ? "N/A" : ban.id);
    }

    public static MutableComponent getModerator() {
        GetBanRequest.Ban ban = BAN_INFO.get();
        return new TextComponent(ban == null ? "N/A" : ban.moderator);
    }

    public static MutableComponent getReason() {
        GetBanRequest.Ban ban = BAN_INFO.get();
        return new TextComponent(ban == null ? "N/A" : ban.reason);
    }

    public static MutableComponent getTimestamp() {
        IsBannedRequest.Ban ban = ACTIVE_BAN.get();
        return new TextComponent(ban == null ? "N/A" : ban.timestamp);
    }

    public static MutableComponent getAppealStatus() {
        GetBanRequest.Ban ban = BAN_INFO.get();
        if (ban == null || ban.appeal == null) {
            return new TranslatableComponent("minetogether:gui.profile.ban.appeal_no_info");
        }
        return new TranslatableComponent("minetogether:gui.profile.ban.appeal_submitted");
    }
    ...
    public static MutableComponent getNextAppealComp() {
        ...
        return nextAppeal == -1 ? new TextComponent("N/A") : new TextComponent(DATE_FORMAT.format(nextAppeal * 1000));
    }
    ...
    public static List<Component> getNotesTooltip() {
        ...
        for (GetBanRequest.Note note : getNotes()) {
            list.add(new TranslatableComponent("minetogether:gui.profile.ban.appeal_note_mod", new TextComponent(note.moderator).withStyle(ChatFormatting.GOLD)).withStyle(ChatFormatting.BLUE));
            list.add(new TextComponent(note.message).withStyle(ChatFormatting.GRAY));
            list.add(new TextComponent(DATE_FORMAT.format(note.timestamp * 1000)).withStyle(ChatFormatting.DARK_GRAY));
        }
        return list;
    }
    ...
    public static MutableComponent getCanChangeComp() {
        return nextChange == -1 ? new TextComponent("N/A") : new TextComponent(DATE_FORMAT.format(nextChange * 1000));
    }
        ...
        var future = CompletableFuture.runAsync(() -> {
            try {
                ...
                if (response.success) {
                    ...
                    errorHandler.accept(new TranslatableComponent("minetogether:gui.profile.error.get_name_fail").withStyle(ChatFormatting.RED));
                    errorHandler.accept(new TextComponent(response.reason).withStyle(ChatFormatting.RED));
                }
            } catch (Throwable e) {
                LOGGER.error("Failed to retrieve current name from API", e);
                errorHandler.accept(new TextComponent("An error occurred while retrieving current name from API").withStyle(ChatFormatting.RED));
            }
        }, EXECUTOR);
        ...
        var future = CompletableFuture.runAsync(() -> {
            try {
                ...
                if (response.success) {
                    ...
                    errorHandler.accept(new TranslatableComponent("minetogether:gui.profile.error.get_names_fail").withStyle(ChatFormatting.RED));
                    errorHandler.accept(new TextComponent(response.reason).withStyle(ChatFormatting.RED));
                }
            } catch (Throwable e) {
                LOGGER.error("Failed to retrieve name options from API", e);
                errorHandler.accept(new TextComponent("An error occurred while retrieving name options from API").withStyle(ChatFormatting.RED));
            }
        }, EXECUTOR);
        ACTIVE_REQUESTS.put(future, onComplete);
    }
        ...
        var future = CompletableFuture.runAsync(() -> {
            try {
                ...
                if ("success".equals(response.getStatus())) {
                    ...
                    ACTIVE_BAN.set(null);
                    errorHandler.accept(new TranslatableComponent("minetogether:gui.profile.error.get_ban_fail").withStyle(ChatFormatting.RED));
                    errorHandler.accept(new TextComponent(response.getMessageOrNull() == null ? "Unknown reason" : response.getMessageOrNull()).withStyle(ChatFormatting.RED));
                }
            } catch (Throwable e) {
                LOGGER.error("Failed to retrieve ban status from API", e);
                errorHandler.accept(new TextComponent("An error occurred while retrieving ban status from API").withStyle(ChatFormatting.RED));
        ...
        var future = CompletableFuture.runAsync(() -> {
            try {
                ...
                if (response.success) {
                    ...
                    errorHandler.accept(new TranslatableComponent("minetogether:gui.profile.error.get_ban_info_fail").withStyle(ChatFormatting.RED));
                    errorHandler.accept(new TextComponent(response.reason).withStyle(ChatFormatting.RED));
                }
            } catch (Throwable e) {
                LOGGER.error("Failed to retrieve ban info from API", e);
                errorHandler.accept(new TextComponent("An error occurred while retrieving ban info from API").withStyle(ChatFormatting.RED));
            }
        }, EXECUTOR);
        ACTIVE_REQUESTS.put(future, onComplete);
    }
    ...
    public static void setName(String newName, Runnable onComplete) {
        if (newName.trim().isEmpty()) {
            errorHandler.accept(new TranslatableComponent("minetogether:gui.profile.name_is_empty").withStyle(ChatFormatting.RED));
            return;
        }

        errorHandler.accept(new TranslatableComponent("minetogether:gui.profile.name_change_sent"));
        var future = CompletableFuture.runAsync(() -> {
            try {
                PutNameRequest.Response response = MineTogetherChat.CHAT_STATE.api.execute(new PutNameRequest(newName)).apiResponse();
                if (response.success) {
                    errorHandler.accept(new TranslatableComponent("minetogether:gui.profile.name_change_success").append(" ").withStyle(ChatFormatting.GREEN).append(new TextComponent(response.name).withStyle(ChatFormatting.GOLD)));
                } else {
                    errorHandler.accept(new TranslatableComponent("minetogether:gui.profile.name_change_fail").withStyle(ChatFormatting.RED));
                    errorHandler.accept(new TextComponent(response.reason).withStyle(ChatFormatting.RED));
                    if (response.nextChange != -1) {
                        errorHandler.accept(new TranslatableComponent("minetogether:gui.profile.next_name_change").withStyle(ChatFormatting.GRAY));
                        errorHandler.accept(new TextComponent(DATE_FORMAT.format(response.nextChange * 1000)).withStyle(ChatFormatting.GRAY));
                    }
                }
            } catch (Throwable e) {
                LOGGER.error("An error occurred while trying to set name", e);
                errorHandler.accept(new TextComponent("An error occurred while trying to set name").withStyle(ChatFormatting.RED));
            }
        }, EXECUTOR);
        ACTIVE_REQUESTS.put(future, onComplete);
    }
    ...
    public static void submitAppeal(String appeal, BiConsumer<Boolean, Component> onComplete) {
        if (appeal.trim().isEmpty()) {
            errorHandler.accept(new TranslatableComponent("minetogether:gui.profile.ban.appeal_is_empty").withStyle(ChatFormatting.RED));
            return;
        }

        AtomicBoolean success = new AtomicBoolean(false);
        AtomicReference<Component> message = new AtomicReference<>();
        errorHandler.accept(new TranslatableComponent("minetogether:gui.profile.ban.submitting"));
        var future = CompletableFuture.runAsync(() -> {
            try {
                Apiv2Response response = MineTogetherChat.CHAT_STATE.api.execute(new DeleteBanRequest(BAN_INFO.get().id, appeal)).apiResponse();
                if (response.success) {
                    success.set(true);
                    errorHandler.accept(new TranslatableComponent("minetogether:gui.profile.ban.submitted").withStyle(ChatFormatting.GREEN));
                    message.set(new TranslatableComponent("minetogether:gui.profile.ban.submitted").withStyle(ChatFormatting.GREEN));
                } else {
                    errorHandler.accept(new TranslatableComponent("minetogether:gui.profile.ban.appeal_fail").withStyle(ChatFormatting.RED));
                    errorHandler.accept(new TextComponent(response.reason).withStyle(ChatFormatting.RED));
                    message.set(new TranslatableComponent("minetogether:gui.profile.ban.appeal_fail").withStyle(ChatFormatting.RED).append("\n" + response.reason));
                }
            } catch (Throwable e) {
                LOGGER.error("An error occurred while trying send ban appeal", e);
                errorHandler.accept(new TextComponent("An error occurred while trying to send ban appeal").withStyle(ChatFormatting.RED));
                message.set(new TextComponent("An error occurred while trying to send ban appeal").withStyle(ChatFormatting.RED));
            }
        }, EXECUTOR);
